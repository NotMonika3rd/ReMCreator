...
import rip.sayori.rmcr.element.*;
...
import rip.sayori.rmcr.ui.action.UnregisteredAction;
import rip.sayori.rmcr.ui.component.*;
import rip.sayori.rmcr.ui.component.util.ComponentUtils;
import rip.sayori.rmcr.ui.component.util.EventButtonGroup;
import rip.sayori.rmcr.ui.component.util.ListUtil;
import rip.sayori.rmcr.ui.component.util.PanelUtils;
import rip.sayori.rmcr.ui.dialogs.ModElementIDsDialog;
...
import rip.sayori.rmcr.ui.laf.renderer.LargeIconModListRender;
import rip.sayori.rmcr.ui.laf.renderer.ListIconModListRender;
import rip.sayori.rmcr.ui.laf.renderer.SmallIconModListRender;
import rip.sayori.rmcr.ui.modgui.ModElementGUI;
import rip.sayori.rmcr.ui.modgui.ModTypeDropdown;
...
import rip.sayori.rmcr.ui.workspace.breadcrumb.WorkspaceFolderBreadcrumb;
import rip.sayori.rmcr.ui.workspace.resources.WorkspacePanelResources;
...
import rip.sayori.rmcr.workspace.elements.IElement;
...
@SuppressWarnings("EqualsBetweenInconvertibleTypes") public class WorkspacePanel extends JPanel {
	...
	public WorkspacePanel(final MCreator mcreator) {
		...
		for (ModElementType type : Arrays.stream(ModElementType.values())
			...
			filterPopup.add(new UnregisteredAction(type.getReadableName(), e -> search.setText(
					"f:" + type.getReadableName().replace(" ", "").toLowerCase(Locale.ENGLISH))).setIcon(
					new ImageIcon(ImageUtils.resizeAA(TiledImageCache.getModTypeIcon(type).getImage(), 16))));
