...
import rip.sayori.rmcr.element.types.Particle;
...
public class ParticleGUI extends ModElementGUI<Particle> {
	...
	@Override protected void initGUI() {
		additionalExpiryCondition = new ProcedureSelector(this.withEntry("particle/additional_expiry_condition"),
				...
				VariableElementType.LOGIC, Dependency.fromString(
				"x:number/y:number/z:number/world:world/age:number/onGround:boolean")).setDefaultName(
		...
		pane3.add("Center", PanelUtils.totalCenterInPanel(
				PanelUtils.centerAndSouthElement(spo2, PanelUtils.join(FlowLayout.LEFT, additionalExpiryCondition), 0,
						10)));
