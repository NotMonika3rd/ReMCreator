...
import rip.sayori.rmcr.ui.component.zoompane.IZoomable;
...
public class WYSIWYG extends JComponent implements MouseMotionListener, MouseListener, IZoomable {
	...
	@Override public void mouseClicked(MouseEvent e) {
		...
		if (positioningMode && e.getButton() == 1) {
			...
			if (e.getButton() == 1 || !(selected instanceof rip.sayori.rmcr.element.parts.gui.SizedComponent)) {
	...
	private void finishGUIComponentMove() {
		for (int i = 0; i < wysiwygEditor.components.size(); i++) {
			...
			if (component.equals(selected)) {
				...
				component.y = oy;
				if (positioningModeSettingWidth
						&& component instanceof rip.sayori.rmcr.element.parts.gui.SizedComponent) {
					((SizedComponent) component).width = ow;
					((rip.sayori.rmcr.element.parts.gui.SizedComponent) component).height = oh;
				}
				break;
			}
		}
