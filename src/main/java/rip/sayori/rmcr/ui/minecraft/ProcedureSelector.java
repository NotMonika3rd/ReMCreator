	...
	public ProcedureSelector(@Nullable IHelpContext helpContext, MCreator mcreator, String eventName, Side side,
		...
		if (returnType == VariableElementType.LOGIC) {
			...
			setBorder(BorderFactory.createLineBorder(
					new Dependency("", VariableElementType.LOGIC.toDependencyType()).getColor()));
		}
		...
		if (side == Side.CLIENT) {
			...
			if (helpContext == null)
				top.add("North", PanelUtils.westAndCenterElement(eventNameLabel,
						ComponentUtils.deriveFont(new JLabel(eventName), 14)));
			else
				top.add("North", PanelUtils.westAndCenterElement(eventNameLabel,
						HelpUtils.wrapWithHelpButton(helpContext, ComponentUtils.deriveFont(new JLabel(eventName), 14),
								SwingConstants.LEFT)));
		...
		if (allowInlineEditor) {
			...
			edit.addActionListener(e -> {
				if (getSelectedProcedure() != null) {
					...
					ModElementGUI<?> modeditor = ModElementTypeRegistry.REGISTRY.get(
									selectedProcedureAsModElement.getType())
